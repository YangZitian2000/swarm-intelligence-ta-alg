...
def co_evolution(interact_time, independent_time, absolute_time, at_valid=False, share_type=1):
    ...
    if at_valid:
        ...
        for iter_time in range(interact_time):  # 交互次数
            ...
            # log = {}
            # log["profit"] = agents[0].local_population_fit[0]
            # sol_all = []
            # best = agents[0].get_best_intact_indi()
            # for a_index in range(AGENT_NUM): # 對每個agent
            #     sol = {}
            #     sol["agent"] = a_index
            #     allo_all = []
            #     for t_index in range(best[0].size):
            #         allo= {}
            #         allo["task"] = t_index
            #         allo["resource"] = best[k][t_index]

            # for k in range(AGENT_NUM):
            #     log = {}
            #     log["agent"] = k
            #     log["profit"] = agents[k].local_population_fit[0]
            #     log["sol"] = []
            #     best = agents[k].get_best_intact_indi()
            #     for a_index in range(best.size):
            #         sol = {}
            #         sol["agent"] = a_index
            #         resource_all = []
            #         for t_index in range(best[0].size):
            #             resource_t = {}
            #             resource_t["agent"] = a_index
...
if __name__ == '__main__':
    ...
    # # 添加第二层键值对
    # nested_json['key2'] = {}
    # nested_json['key2']['key3'] = 'value3'
    #
    # # 添加第三层键值对
    # nested_json['key2']['key4'] = {}
    # nested_json['key2']['key4']['key5'] = 'value5'
    # json_data = json.dumps(nested_json ,indent=2)
    #
    # # 输出生成的JSON
    # print(json_data)
    ...
    avg_popu_fitness_all_1, best_popu_fitness_all_1, paralle_compute_1, communication_1 = \
        co_evolution(INTERACT_TIMES, INDEPENDENT_TIMES, 0, False, share_type=1)
    # 第2个图：折线图
    # plt.subplot(2, 1, 2)
    plt.title("random_share")
    # 画分布式
    j = 0
    for best_popu_fit in best_popu_fitness_all_1:
        # X = [i[0] for i in best_popu_fit]
        X = np.arange(0, len(best_popu_fit))
        Y = [i[1] for i in best_popu_fit]
        plt.plot(X, Y, color=cnames[j], label="agent%d best_fit" % j)
        j += 1
