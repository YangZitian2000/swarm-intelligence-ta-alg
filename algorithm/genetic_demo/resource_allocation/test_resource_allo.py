from centralized_resource_allo import *
from ma_resource_allo import *
...
def test_time_consume_in_same_iter_times():
  ...
  print('分布式算法单机运行用时：%0.2f' % (np.mean(distributed_compute_time) * AGENT_NUM +
                               np.mean(distributed_communicate_time)))
  print('分布式算法并行运行用时：%0.2f' % (np.mean(distributed_compute_time) +
                               np.mean(distributed_communicate_time)))
