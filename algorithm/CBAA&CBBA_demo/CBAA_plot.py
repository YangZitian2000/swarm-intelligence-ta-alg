...
while True:
  ...
  for robot_id, robot in enumerate(robot_list):
    ...
    if t == 0:
      assign_line, = ax.plot([robot.state[0][0], task[robot.J, 0]], [
                             robot.state[0][1], task[robot.J, 1]], 'k-', linewidth=1)
      assign_plots.append(assign_line)
    else:
      assign_plots[robot_id].set_data([robot.state[0][0], task[robot.J, 0]], [
                                      robot.state[0][1], task[robot.J, 1]])
  ...
  for robot_id, robot in enumerate(robot_list):
    ...
    if np.array(robot.x).any():
      assign_plots[robot_id].set_data([robot.state[0][0], task[robot.J, 0]], [
                                      robot.state[0][1], task[robot.J, 1]])
    else:
      assign_plots[robot_id].set_data([robot.state[0][0], robot.state[0][0]], [
                                      robot.state[0][1], robot.state[0][1]])
